...
/*
All leaf nodes of btree are in the same level.
*/
...
// max elements in a node
const int max_elements = 5;

// size limit for the array in a vector object.
// best performance was at 800 bytes.
...
class Element {
 ...
 public:
  ...
  bool valid() const {
    return mp_subtree != invalid_ptr;
  }

  void invalidate() {
    mp_subtree = invalid_ptr;
  }
...
template<class key, class payload>
void Element<key, payload>::dump () {
  LOG(INFO) << "[key = " << m_key << " value: " << m_payload << " sub= " << mp_subtree << "]";
}
...
class RootTracker {
 ...
 public:
  ...
  void set_root (Node* old_root, Node* new_root) {
    ...
    if (old_root != mp_root) throw "wrong old_root in RootTracker::set_root";
...
int Node::minimum_keys () {
  ...
  if (ceiling_func * 2 < size - 1) ceiling_func++;
...
inline void Node::set_debug() {
  ...
  for (i = 0; i < m_count && i < 8; i++) {
    debug[i] = m_vector[i];
    if (m_vector[i].mp_subtree) m_vector[i].mp_subtree->set_debug();
  }
  for (; i < 8; i++) debug[i] = m_failure;
#endif
}
...
void Node::dump() {
  ...
  if (this == m_root.get_root()) LOG(INFO) << "ROOT";
  LOG(INFO) << "this =" << this;
  LOG(INFO) << "parent = " << mp_parent << " count = " << m_count - 1;

  for (int i = 1; i < m_count; i++) m_vector[i].dump();
  HERE(INFO);
  for (int i = 0; i < m_count; i++) {
    if (m_vector[i].mp_subtree) m_vector[i].mp_subtree->dump();
  }
}
...
Node* Node::find_root() {
  Node* current = this;
  while (current->mp_parent) current = current->mp_parent;
  return current;
}
...
bool Node::vector_insert(Elem& element) {
  // leave an extra slot for split_insert
  if (m_count >= m_vector.size() - 1) return false;
  int i = m_count;
...
bool Node::vector_insert_for_split(Elem& element) {
  if (m_count >= m_vector.size()) return false;
  int i = m_count;
  while (i > 0 && m_vector[i - 1] > element) {
    m_vector[i] = m_vector[i - 1];
    i--;
...
bool Node::split_insert(Elem& element) {
  if (m_count != m_vector.size() - 1) throw "bad m_count in split_insert";
  vector_insert_for_split(element);
  int split_point = m_count / 2;
  if (2 * split_point < m_count) split_point++;
  ...
  for (int i = 1; i < m_count - split_point; i++) new_node->vector_insert(m_vector[split_point + i]);
  ...
  if (mp_parent && mp_parent->vector_insert(upward_element)) return true;
  else if (mp_parent && mp_parent->split_insert(upward_element)) return true;
...
bool Node::tree_insert(Elem& element) {
    ...
    if (search(element, last_visited_ptr).valid()) return false;
    if (last_visited_ptr->vector_insert(element)) return true;
...
bool Node::delete_element(Elem& target) {
  ...
  if (node->is_leaf() && node->key_count() > node->minimum_keys()) return node->vector_delete (target);
    ...
    while (node) {
      ...
      if (node == node->find_root() && node->is_leaf()) break;
...
Node* Node::merge_right(int parent_index_this) {
  ...
  for (int i = 1; i < right_sib->m_count; i++) vector_insert ((*right_sib)[i]);
  ...
  if (mp_parent==find_root() && !mp_parent->key_count()) {
    ...
    mp_parent = 0;
    return null_ptr;
  } else if (mp_parent==find_root() && mp_parent->key_count()) return null_ptr;

  // no need for parent to import an element
  if (mp_parent&& mp_parent->key_count() >= mp_parent->minimum_keys()) return null_ptr;
...
Node* Node::merge_left (int parent_index_this) {
  ...
  for (int i = 1; i < m_count; i++) left_sib->vector_insert (m_vector[i]);
...
Node* Node::right_sibling (int& parent_index_this) {
  ...
  if (parent_index_this == invalid_index) return 0;
  // now mp_parent is known not to be null
  if (parent_index_this >= mp_parent->m_count - 1) return 0;  // no right sibling
...
Node* Node::left_sibling (int& parent_index_this) {
  ...
  if (parent_index_this == invalid_index) return 0;
  // now mp_parent is known not to be null
  if (parent_index_this == 0) return 0;  // no left sibling
  return mp_parent->m_vector[parent_index_this - 1].mp_subtree;  // might be null
}

// return the element in this node's parent that has the "this" pointer as its subtree
int Node::index_has_subtree () {
  if (!mp_parent) return invalid_index;
  int first = 0;
  int last = mp_parent->m_count - 1;
  while (last - first > 1) {
    int mid = first + (last - first) / 2;
    Elem& smallest = smallest_key();
    if (smallest >= mp_parent->m_vector[mid]) first = mid;
    else last = mid;
  }
 if (mp_parent->m_vector[first].mp_subtree == this) return first;
 else if (mp_parent->m_vector[last].mp_subtree == this) return last;
 else throw "error in index_has_subtree";
}
...
Elem& Node::search (Elem & desired, Node * & last_visited_ptr) {
  ...
  while (current) {
    ...
    if (current->m_count > 1 && desired<current->m_vector[1]) {
      ...
      else if (current->m_vector[last] > desired) current = current->m_vector[first].mp_subtree;
      else current = current->m_vector[last].mp_subtree;
    }
  }
  return m_failure;
}
...
int main(int argc, char** argv) {
  ...
  Elem elem;
  RootTracker tracker;
  // maintains a pointer to the current root of the b-tree
  ...
  for (int i = 0; i < 10; i++) {
    ...
    tracker.get_root()->search(desired, last);
  }
