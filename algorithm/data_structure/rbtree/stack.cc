...
stk_stack * StackCreate() {
  stk_stack * newStack = (stk_stack *) SafeMalloc(sizeof(stk_stack));
  newStack->top = NULL;
  newStack->tail = NULL;
  return newStack;
}
...
void StackPush(stk_stack * theStack, DATA_TYPE newInfoPointer) {
  stk_stack_node * newNode = (stk_stack_node *) SafeMalloc(sizeof(stk_stack_node));
  newNode->info = newInfoPointer;
  newNode->next = theStack->top;
  theStack->top = newNode;
  if (!theStack->tail) theStack->tail = newNode;
}
