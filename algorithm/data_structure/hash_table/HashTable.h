 ...
 public:
  ...
  bool operator == (const HashTableIterator& other) {
    if (other._currentEntry == _currentEntry &&
        other._currentBucket == _currentBucket) return true;
    return false;
  }

  bool operator != (const HashTableIterator& other) {
    if (other._currentEntry == _currentEntry &&
        other._currentBucket == _currentBucket) return false;
    return true;
  }
...
class HashTable {
 ...
 private:
  ...
  // if allocator != NULL there will be space waste,
  // because the space of b can not be used again.
  void _deleteBucket(bucket_type* b) {
    if (!_allocator) delete b;
  }
