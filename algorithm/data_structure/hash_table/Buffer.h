...
class Buffer {
 ...
 public:
  Buffer(size_t length) : _buffer((char*) malloc(length)),
                          _size(length), _position(0) {}
  Buffer() : _buffer(0), _size(0), _position(0) {}
  ...
  inline char* write(size_t length) {
    if (_position + length > _size) grow(_position + length);
