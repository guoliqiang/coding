 ...
 *
 *   1.构造附加网络(不添加[t,s]边)
 *     超级源点ss连i（du[i]>0），i连超级汇点（du[i]<0）
 *   2.对ss、tt求最大流 (http://www.cnblogs.com/kane0526/archive/2013/04/05/3001108.html 里面有解释为什么先做一次最大流, 可能有环,
 *                       这个步骤可以先让环满流)
 *   3.添加[t,s]边
 *   4.对ss、tt求最大流
 *   5.若ss、tt满流，则[t,s]的流量就是最小流, 否则不存在这样的流
 *
 *   poj3801 (下面的写法有问题，正确写法见poj3801)
 *
 */
...
void MinFlowWithBound(int source, int target,
