...
class Kernel: public QMatrix {
 ...
 private:
	static double dot(const svm_node *px,
                    const svm_node *py);
	double kernel_linear(int i,
                       int j) const {
		return dot(x[i], x[j]);
	}

	double kernel_poly(int i,
                     int j) const {
	...
	double kernel_sigmoid(int i,
                        int j) const {
		return tanh(gamma*dot(x[i], x[j]) + coef0);
	}

	double kernel_precomputed(int i,
                            int j) const {
...
class Solver_NU: public Solver {
 ...
 private:
	...
	bool be_shrunk(int i, double Gmax1,
                 double Gmax2, double Gmax3,
                 double Gmax4);
	void do_shrinking();
};
...
bool Solver_NU::be_shrunk(int i, double Gmax1,
                          double Gmax2, double Gmax3,
                          double Gmax4) {
	if(is_upper_bound(i)) {
		if(y[i] == +1) {
...
class SVC_Q: public Kernel { 
 public:
	SVC_Q(const svm_problem& prob,
        const svm_parameter& param,
        const schar *y_) :Kernel(prob.l, prob.x, param) {
		clone(y, y_, prob.l);
...
class ONE_CLASS_Q: public Kernel {
 public:
	ONE_CLASS_Q(const svm_problem& prob,
              const svm_parameter& param) :Kernel(prob.l, prob.x, param) {
		cache = new Cache(prob.l,(long int)(param.cache_size*(1<<20)));
		QD = new double[prob.l];
		for(int i=0;i<prob.l;i++) {
			QD[i] = (this->*kernel_function)(i,i);
...
static double svm_svr_probability(const svm_problem *prob,
	...
	mae /= (prob->l-count);
	info("Prob. model for test data: target value = predicted"
...
svm_model *svm_train(const svm_problem *prob,
	...
	if(param->svm_type == ONE_CLASS ||
	   ...
	   param->svm_type == NU_SVR) {
		...
		if(nr_class == 1)  info("WARNING: training data in only"
...
const char *svm_check_parameter(const svm_problem *prob,
	...
	if(svm_type != C_SVC &&
	   ...
	   svm_type != NU_SVR) return "unknown svm type";
	...
	if(kernel_type != LINEAR &&
	   ...
	   kernel_type != RBF &&
	   kernel_type != SIGMOID &&
	   kernel_type != PRECOMPUTED) return "unknown kernel type";
	...
	if(param->shrinking != 0 &&
	   param->shrinking != 1) return "shrinking != 0 and shrinking != 1";

	if(param->probability != 0 &&
	   param->probability != 1) return "probability != 0 and probability != 1";

	if(param->probability == 1 &&
	   svm_type == ONE_CLASS) return "one-class SVM probability output not supported yet";
