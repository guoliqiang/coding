...
SMO::SMO(const std::vector<base::shared_ptr<ProblemNode> > & a,
  ...
  G_.assign(node_count_, 0);
  p_.assign(node_count_, 0);
  for (int i = 0; i < node_count_; i++) {
    if (i < a_.size()) {
      G_[i] = para_->p_ - GetNode(i).value;
    } else {
      G_[i] = para_->p_ + GetNode(i).value;
    }
    p_[i] = G_[i];
...
base::shared_ptr<CacheNode> SMO::GetQ(int32_t i, int32_t len) {
  ...
  int sign_i = i < node_count_ / 2 ? 1 : -1;
  std::string debug = "";
  for (int j = start; j < len; j++) {
    int sign_j = j < node_count_ / 2 ? 1 : -1;
    rs.get()->data[j] = sign_i * sign_j *
        Kernel::GetInstance().Do(GetNode(i), GetNode(j));
    VLOG(5) << "sign[" << i << "]=" << sign_i << " sign[" << j << "]="
            << sign_j << " kernel(x[" << i << "],x[" << j << "])="
            << Kernel::GetInstance().Do(GetNode(i), GetNode(j))
            << "\nx[" << i << "]:\n" << GetNode(i).ToString()
            << "\nx[" << j << "]:\n" << GetNode(j).ToString();
    debug += "value[" + IntToString(j) + "]=" +
             DoubleToString(rs.get()->data[j]) + " ";
  }
  VLOG(5) << "GetQ " << debug;
  return rs;
}
...
void SMO::Do(ModelNode * ptr) {
  ...
  while (iter < max_iter) {
    ...
    VLOG(4) << "alpha[" << i << "]=" << alpha_[i] << " alpha_[" << j
            << "]=" << alpha_[j];
  ...
  double v = 0;
  VLOG(4) << "G=" << JoinVectorD(G_);
  VLOG(4) << "alpha=" << JoinVectorD(alpha_);
  VLOG(4) << "p=" << JoinVectorD(p_);
  for (int i = 0; i < node_count_; i++) {
  ...
  // when predict x(m)
  // sum((alpha_star[i] - alpha_[i]) * kernel(x[i], x[m])) + b

  // put back the solution
  for (int i = 0; i < node_count_ / 2; i++) {
    ptr->alpha.insert(i, alpha_[i] - alpha_[i + node_count_ / 2]);
  }
}
}   // namepace svr
}  // namespace nltk
