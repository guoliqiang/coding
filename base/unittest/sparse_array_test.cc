...
#include "base/public/sparse_array.h"
...
template <class Type>
void InsertOrUpdate(SparseArray<Type> * array, int index, Type value) {
  array->set(index, value);
}

template <class Type>
bool ContainsKey(SparseArray<Type> & array, int index) {
  return array.find(index) != array.end();
}

template <class Type>
bool InsertIfNotPresent(SparseArray<Type> * array, int index, Type value) {
  if (ContainsKey(*array, index)) return false;
  return array->set_new(index, value) != array->begin();
}

template <class Type>
Type FindWithDefault(SparseArray<Type>& array, int index, Type value) {
...
class SparseArrayStringTest : public testing::Test {
 protected:
  SparseArrayStringTest()
      : str_map_(10) {
    InsertOrUpdate<string>(&str_map_, 1, "a");
    InsertOrUpdate<string>(&str_map_, 5, "b");
    InsertOrUpdate<string>(&str_map_, 2, "c");
    InsertOrUpdate<string>(&str_map_, 7, "d");
  }
...
TEST_F(SparseArrayStringTest, InsertIfNotPresent) {
  EXPECT_FALSE(ContainsKey(str_map_, 3));
  EXPECT_TRUE(InsertIfNotPresent<string>(&str_map_, 3, "r"));
  EXPECT_EQ("r", FindWithDefault(str_map_, 3, kNotFound));
  EXPECT_FALSE(InsertIfNotPresent<string>(&str_map_, 3, "other value"));
  EXPECT_EQ("r", FindWithDefault(str_map_, 3, kNotFound));
}
