...
//
// Used in the status for high efficient but not saving memory requried.
//
// NOte: in order to save memory, need to delete  "dense_.resize(max_size);", 
// if there are k instances the used sizeof(int) * m + k * sizeof(T)
// (k < m)
// 
...
#ifndef SPARSE_ARRAY_H__
#define SPARSE_ARRAY_H__
...
template<typename Value>
SparseArray<Value>::SparseArray(): size_(0), max_size_(0),
                                   sparse_to_dense_(NULL), dense_() {}
