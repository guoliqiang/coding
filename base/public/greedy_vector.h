...
class greedy_vector {
 private:
  ...
  char _stack[ sizeof(_Type) * _Count ] __attribute__ ((aligned));
  _Type* _array;
  size_t _size;
  size_t _space;
  ...
  void _allocate(size_t newSpace) {
    ...
    if (newBuffer == NULL) throw new std::bad_alloc();
    if (_size) {
    ...
    if (_array != (_Type*) _stack) free(_array);
    _array = newBuffer;
     _space = newSpace;
  }
  ...
  void _grow(size_t newSize, const _Type& prototype) {
    ...
    while (_space < newSize) _more_space();
  ...
  public:
   ...
   greedy_vector() : _array((_Type*) _stack), _size(0), _space(_Count) {}

   template<typename _VectorType>
   greedy_vector(const _VectorType& other) : _array((_Type*) _stack), _size(0),
                                             _space(_Count) {
     assign(other.begin(), other.end());
   }
  
   greedy_vector(const my_type& other) : _array((_Type*) _stack),
                                         _size(0), _space(_Count) {
     assign(other.begin(), other.end());
   }

   explicit greedy_vector(size_t storageCount) : _array((_Type*) _stack),
                                                 _size(0),
                                                 _space(_Count) {
     if (_space < storageCount) _allocate(storageCount);
   }
   ...
   void reserve(size_t space) {
     if (space > _space) _allocate(space);
   }
  ...
  reference at(size_t index) {
    if (index >= _size) _subscript_gripe();
    return *(_array + index);
  }
  ...
  void append(const _Iter otherBegin, const _Iter otherEnd) {
    ...
    while (_size + size_t(otherEnd - otherBegin) > _space) _more_space();
