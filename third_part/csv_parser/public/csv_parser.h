...
// @li ENCLOSURE_NONE
//   means the CSV file does not use any enclosure characters for the fields
// @li ENCLOSURE_REQUIRED
//   means the CSV file requires enclosure characters for all the fields
// @li ENCLOSURE_OPTIONAL
//   means the use of enclosure characters for the fields is optional
...
// @li The parser can handle documents where fields are always enclosed,
// not enclosed at all or optionally enclosed.
...
// character is spotted at either the beginning
// or the end of the string, it is assumed that the field is enclosed.
// The csv_parser::init() method can accept a character array as the path to
// the CSV file.
// Since it is overloaded, it can also accept a FILE pointer to a stream that
// is already open for reading.
// The set_enclosed_char() method accepts the field enclosure character as the
// first parameter and the enclosure mode as the second parameter which
...
// @todo Add ability to set strings where lines start by. Currently lines do
// not have any starting char or string.
...
class csv_parser {
 public :
  ...
  // and 1 respectively.
	...
	// @li The pointer to the buffer for the file name is also initialized
  // to NULL
  ...
  // and the buffer to the input file name is also deallocated.
	...
	// This init method accepts a pointer to the CSV file that has
  // been opened for reading
	...
	// If the enclosure is optional, there could be fields that are enclosed,
  // and fields that are not enclosed within the same line/record.
	...
	// Call this method to see if there are more rows to retrieve before
  // invoking csv_parser::get_row()
	// @see csv_parser::get_row()
	// @see csv_parser::more_rows
	// @return bool Returns true if there are still more rows and false
  // if there is not.
	...
	// Returns the number of times the csv_parser::get_row() method has
  // been invoked
	// @see csv_parser::reset_record_count()
	// @return unsigned int The number of times the csv_parser::get_row()
 ...
 private :
	...
	// @param[out] buffer_len A pointer to an integer storing the length of
  // the buffer.
	// @return void
	...
	// @param[out] row The vector of strings
	// @param[in] line The character array buffer containing the
  // current record/line
	// @param[in] line_length The length of the buffer
  ...
  // record/line
	// @param[in] line_length The length of the buffer
	void _get_fields_with_optional_enclosure(csv_row_ptr row,
 ...
 protected :
	...
	// For now the only valid escape character allowed is the backslash
  // character 0x5C
	// This is only important when the enclosure character is
  // required or optional.
	// This is the backslash character used to escape enclosure characters
  // found within the fields.
	// @see csv_parser::_get_fields_with_enclosure()
	// @see csv_parser::_get_fields_with_optional_enclosure()
	// @todo Update the code to accept other escape characters
  // besides the backslash
	// @var escaped_char
	char escaped_char;

	// The field terminator
	// This is the single character used to mark the end of a column in
  // the text file.
  ...
  // in the text file.
	// The most popular one is the new line character however it is possible
  // to use others as well.
	// This is the single character used to mark the end of a record
	// @see csv_parser::get_row()
	// @var line_term_char
	char line_term_char;
	...
	// @todo Update the code to accept other escape
  // characters besides the backslash
	// @var escaped_length
	unsigned int escaped_length;

	// Length of the field terminator
	// For now this is not being used.
  // It will be used in future versions of the object.
  ...
  // It will be used in future versions of the object.
	...
	// This variable controls how many records in the file are skipped
