...
//
// * Each level is sorted by a different criteria
...
//    to the right So, KDTree has the relationships:
//    * The left branch is <= its parent
//      (in binary tree, this relationship is a plain < )
//    * The right branch is <= its parent (same as binary tree)
//
//    This is done for mostly for performance.
//    Its a LOT easier to maintain a consistent tree if we use
//    the <= relationship. Note that this relationship only makes a difference
//    when searching for an exact item with find() or find_exact, other search,
//    erase and insert functions don't notice the difference.
//
//    In the case of binary trees, you can safely assume that the next identical
//    item  will be the child leaf, but in the case of KDTree, the next
//    identical item might be a long way down a subtree, because of the various
...
//        following 1 branch, we must use !compare(node,child) for
//        a <= relationship, and test BOTH branches, as we could potentially
...
#include"third_part/kdtree++/public/region.h"
...
class KDTree : protected _Alloc_base<_Val, _Alloc> {
 ...
 public:
  ...
  void efficient_replace_and_optimise(
  ...
  // Note: the static_cast in end() is invalid (_M_header is not convertable
  // to a _Link_type), but thats ok as it just means undefined behaviour if the
  // user dereferences the end() iterator.
  ...
  // NOTE: this will visit points based on 'Manhattan distance' aka city-block
  // distance aka taxicab metric. Meaning it will find all points within:
  //    max(x_dist,max(y_dist,z_dist));
  //  AND NOT than what you would expect:
  //    sqrt(x_dist*x_dist + y_dist*y_dist + z_dist*z_dist)
  //
  // This is because it converts the distance into a bounding-box 'region' and
 ...
 protected:
  void _M_check_children(_Link_const_type child,
    ...
    if (child) {
      ...
      // check the left
      assert(!to_the_left || !compare(parent->_M_value,child->_M_value));
      // check the right
      assert(to_the_left || !compare(child->_M_value,parent->_M_value));
  ...
  _Link_type _M_get_erase_replacement(_Link_type node, size_type const level) {
    ...
    // we have both children ...
      ...
      // left or right branch RANDOMLY, so that the tree has a greater change
      // of staying balanced.
      // If this were a true binary tree, we would always hunt down the right
      // branch.See top for notes.
      _Node_compare_ compare(level % __K, _M_acc, _M_cmp);
  ...
  const_iterator _M_find(_Link_const_type node, const_reference value,
    ...
    // be aware! This is very different to normal binary searches, because of
    // the <= relationship used. See top for notes.
  ...
  const_iterator _M_find_exact(_Link_const_type node,
      const_reference value, size_type const level) const {
    // be aware! This is very different to normal binary searches, because of
    // the <= relationship used. See top for notes.
