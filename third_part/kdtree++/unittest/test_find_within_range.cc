...
int main(int argc, char *argv[]) {
  ...
  for (size_t n = 0; n < 10000; ++n) {
    kdtreeNode node;
    node.xyz[0] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    node.xyz[1] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    node.xyz[2] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    node.index = n;
    tree.insert(node);
    pts.push_back(node);
  }
  for (size_t r = 0; r < 1000; ++r) {
    kdtreeNode refNode;
    refNode.xyz[0] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    refNode.xyz[1] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    refNode.xyz[2] = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
    double limit = static_cast<double>(rand()) / RAND_MAX;  // NOLINT
