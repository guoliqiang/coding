...
#include"third_part/kdtree++/public/kdtree.h"
#include<deque>
#include<iostream>
#include<vector>
#include<limits>
...
// used to ensure all triplets that are accessed via the
// operator<< are initialised.
...
typedef KDTree::KDTree<3, triplet,
    std::pointer_to_binary_function<triplet,size_t,double> > tree_type;
...
int main() {
  ...
  // do the same test, except use alternate_triplet as the search key
  {
    ...
    std::cout << "Test with alternate search type, found: "
              << *found.first << ", wanted " << actual_target << std::endl;
...

  {
    ...
    assert(found.first != exact_dist.end());
    std::cout << "Test find_nearest(), found at exact distance away from "
              << target << ", found " << *found.first << " @ "
              << found.second << " should be " << std::sqrt(8) << std::endl;
    assert(found.second == std::sqrt(8));
  }
  ...
  for (int loop = 0; loop != 4; ++loop) {
    ...
    i=0;
    for (tree_type::const_reverse_iterator iter=t.rbegin();
         iter!=t.rend(); ++iter, ++i);
    std::cout << "reverse_iterator walked through " << i
...

    {
      ...
      // when -O2 or -O3 is switched on. Some sort of optimisation makes the
      // math inexact.
  ...
  // the specified range... this is the test.
  {
    ...
    std::cout << std::endl << "Test find_with_range( " << sv
              << ", 10.0f) found " << vectors.size()
              << " candidates." << std::endl;

    // double-check the ranges
    for (std::deque<triplet>::iterator v = vectors.begin();
         v != vectors.end(); ++v) {
      double dist = sv.distance_to(*v);
      std::cout << "  " << *v << " dist=" << dist << std::endl;
      if (dist > 10.0f)
        std::cout << "    This point is too far! But that is by design, "
                     "its within a 'box' with a 'radius' of 10, not a sphere "
                     "with a radius of 10" << std::endl;
