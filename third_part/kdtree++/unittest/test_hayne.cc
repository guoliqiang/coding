...
int main() {
  ...
  for (int i=0; i<700; i++) {
    ...
    //check if duplet with same coordinate already in vector/tree.
    //If not: insert in vector and tree
    duplet_tree_type::iterator pItr = dupl_tree_test.find_nearest(
        super_dupre,std::numeric_limits<double>::max()).first;
    if (*pItr!=super_dupre) {
  ...
  size_t elements;
  //delete all duplets from tree which are in the vector
  while (vDuplets.size() > 0) {
    ...
    duplet_tree_type::iterator should =
        dupl_tree_test.find_exact(element_to_erase);
    ...
    if (find(vDuplets.begin(),vDuplets.end(),element_to_erase) ==
        vDuplets.end()) {
      duplet_tree_type::iterator not_there =
          dupl_tree_test.find(element_to_erase);
