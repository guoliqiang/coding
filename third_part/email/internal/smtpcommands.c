...
#include "third_part/email/public/dlib/dnet.h"
#include "third_part/email/public/dlib/dstrbuf.h"
...
static int
writeResponse(dsocket *sd, const char *line, ...)
{
	va_list vp;
...
helo(dsocket *sd, const char *domain)
{
	 ...
	 */
	if (writeResponse(sd, (char *)"HELO %s\r\n", domain) < 0) {
...
ehlo(dsocket *sd, const char *domain)
{
	...
	if (writeResponse(sd, (char *)"EHLO %s\r\n", domain) < 0) {
		smtpSetErr("Lost connection to SMTP server");
		retval = ERROR;
		goto end;
	}
...
mailFrom(dsocket *sd, const char *email)
{
	...
	if (writeResponse(sd, (char *)"MAIL FROM:<%s>\r\n", email) < 0) {
		smtpSetErr("Lost connection with SMTP server");
		retval = ERROR;
		goto end;
	}
...
rcpt(dsocket *sd, const char *email)
{
	...
	if (writeResponse(sd, (char *)"RCPT TO: <%s>\r\n", email) < 0) {
		smtpSetErr("Lost connection with SMTP server");
		retval = ERROR;
		goto end;
	}
