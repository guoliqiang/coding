...
const struct node_s* conhash_lookup(const struct conhash_s *conhash,
  ...
  if ((conhash==NULL) || (conhash->ivnodes==0) || (object==NULL)) return NULL;
  // calc hash value
  hash = conhash->cb_hashfunc(object);
  rbnode = util_rbtree_lookup(
      const_cast<util_rbtree_t*>(&(conhash->vnode_tree)), hash);
  if (rbnode != NULL) {
