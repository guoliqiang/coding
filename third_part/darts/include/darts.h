...
template <class T>
inline T* _resize(T* ptr,    // ptr
                  size_t n,  // old size
                  size_t l,  // new size
                  T v) {     // value for (new-old) element
  T *tmp = new T[l];
...
//
template <class node_type_,  // == key_type
          class node_u_type_,
          class array_type_, // == value_type / result_type
          class array_u_type_,
...
class DoubleArrayImpl {
 public:
  ...
  int build(size_t     key_size,         // 字符串个数
            const key_type   **key,      // 字符串数组指针
            const size_t     *length = 0,// 自己设置长度
            const value_type *value = 0, // 自己设定末端base的值
    ...
    insert(siblings);  // return value must == 1
 ...
 private:
  ...
  size_t fetch(const node_t &parent, std::vector <node_t> &siblings) {
     ...
     *  1 "aa"
     *  2 "ab"
     *  3 "ac"  siblings[0]: depth=1 left=0 right=4
     *
     *  4 "b"
     *  5 "bc"  siblings[1]: depth=1 left=4 right=6
     *
     *  6 "cd"
     *  7 "cf"  siblings[2]: depth=1 left=6 right=8
     */
    for (size_t i = parent.left; // 子串中最小的id
         i < parent.right;       // 子串中最大的id
         ++i) {
      ...
      if (cur != prev || siblings.empty()) {
          ...
          tmp_node.code  = cur;  // note: cur = (array_u_type_)tmp[parent.depth] + 1;
          tmp_node.left  = i;
