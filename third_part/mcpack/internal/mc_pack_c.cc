#include "../public/mc_pack_bits/mc_pack2.h"
#include "../public/mc_pack_bits/mc_pack1.h"
    ...
    if (item->type == MC_PT_OBJ || item->type == MC_PT_ARR) {
      ...
      if(item->type == MC_PT_OBJ) {
        if(mc_pack_get_type(dst) == MC_PT_OBJ) {
          tmp = mc_pack_put_object(dst, mc_pack_get_subkey(item->key));
        ...
        if(mc_pack_get_type(dst) == MC_PT_OBJ) {
          tmp = mc_pack_put_array(dst, mc_pack_get_subkey(item->key));
      ...
      newitem.key = mc_pack_get_subkey(item->key);
