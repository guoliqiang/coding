...
mc_pack_json2pack(const char *txt, mc_pack_t *pack) {
    ...
    if (IS_OBJECT(token_q, 0)) {
        if(mc_pack_get_type(pack) == MC_PT_OBJ) {
            ret = match_object(pack, token_q, 0, tok_cnt - 1);
        } else {
            ret = -1;
        }
    } else if (IS_ARRAY(token_q, 0)) {
        if(mc_pack_get_type(pack) == MC_PT_ARR) {
            ret = match_array(pack, token_q, 0, tok_cnt - 1);
        } else {
            ret = -1;
        }
    }
...
mc_pack_pack2json(const mc_pack_t *pack, char *txt_buf, u_int buf_size) {
    ...
    } else if(mc_pack_get_type(pack) == MC_PT_ARR) {
        used += snprintf(txt_buf + used, buf_size - used, "[");
        if (used < buf_size) {
            used += output_pack(pack, txt_buf + used, buf_size - used, 1);
            if (used < buf_size) {
                used += snprintf(txt_buf + used, buf_size - used, "]");
            }
        }
...
int mc_pack_print(const mc_pack_t * src) {
    char buf[4096];
    int result = mc_pack_pack2json(src, buf, sizeof(buf));
    if(result) {
        printf("%s\n", mc_pack_perror(result));
    } else {
        printf("%s\n", buf);
    }
    return 0;
...
int mc_pack_printbuf(const char * packbuf, unsigned int length) {
    char buf[4096];
    char tmpbuf[4096];
    const mc_pack_t * src = mc_pack_open_r(packbuf, length, tmpbuf, sizeof(tmpbuf));
    int result = mc_pack_pack2json(src, buf, sizeof(buf));
    if(result) {
        printf("%s\n", mc_pack_perror(result));
    } else {
