...
public:
    ...
    /**
     * @brief 这个函数是foreach的扩展，是为了解决mcpack2不能够获得key名字全路径所引入的
     * @param cb 回调函数
     * @param arg 回调函数使用附加参数
     * @param key 全路径key
     * @return >=0 表示返回正常，否则表示失败.
     */
...
int Protocol_v2<alloc_t>::foreach_ex(mc_pack_foreach_callback_ex cb, void *arg,full_key_t *key) {
    ...
    while(MC_PE_SUCCESS == result) {
        ...
        cnt=snprintf(buf+buflen,bufsize-buflen,"%s",item.key); //组合成全路径名称
        key->buflen+=cnt; //同时需要修改key的长度..
        ret = cb(&item, arg,key);
        key->buflen-=cnt;
        buf[buflen]=0; //修改回来
