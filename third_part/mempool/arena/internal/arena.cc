...
UnsafeArena::~UnsafeArena() {
  FreeBlocks();
  CHECK(overflow_blocks_ == NULL);  // FreeBlocks() should do that
...
void UnsafeArena::FreeBlocks() {
  ...
  for ( int i = 1; i < blocks_alloced_; ++i ) {
    free(first_blocks_[i].mem);
