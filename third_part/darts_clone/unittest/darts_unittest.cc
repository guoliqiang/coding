...
template <typename T>
void TestDic(const T &dic, const std::vector<const char *> &keys,
...
void TestDarts(const std::set<std::string> &valid_keys,
  ...
  TestDic(dic, keys, lengths, values, invalid_keys);
  ...
  TestDic(dic_copy, keys, lengths, values, invalid_keys);
...
int main() {
  try {
    ...
    TestDarts<Darts::DoubleArray>(valid_keys, invalid_keys);
    TestDarts<Darts::DoubleArrayImpl<char, unsigned char, long,
