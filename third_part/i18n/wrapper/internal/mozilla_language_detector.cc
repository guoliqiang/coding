...
#include "third_part/i18n/language_detector/common.h"
#include "third_part/i18n/language_detector/textcat.h"
#include "../public/encoding.h"
#include "third_part/release/public/release_path.h"

DEFINE_string(language_data_path, "third_part/i18n/LM", "data file folder path");

namespace i18n {

MozillaLanguageDetector::MozillaLanguageDetector() {
  std::string base_directory =
    release::GetReleaseDataPath(FLAGS_language_data_path);
  LOG(INFO) << base_directory;
...
i18n::Language MozillaLanguageDetector::DetectLanguage(const std::string
                                                       &content) {
...
i18n::Language MozillaLanguageDetector::DetectLanguageForUTF8(
  ...
  if (strcmp("english", result) == 0) {
    ...
    lan = i18n::RU;
  } else if (strcmp("arabic-iso8859_6", result) == 0 ||
             strcmp("arabic-windows1256", result) == 0) {
    lan = i18n::AR_SA;
