...
void Result_set::digest_row_set() {
  ...
  try {
    do {
      // Get server response
      packet_length = system::proto_get_one_package(m_socket,
          resultbuff, &packet_no);
        ...
        case FIELD_PACKETS:
          {
            ...
            if (++field_count == m_field_count) m_current_state = MARKER;
          }
          break;
        case MARKER:
         {
       ...
       case ROW_CONTENTS:
         {
           bool is_eof= false;
           Row_of_fields row(0);
           system::digest_row_content(response_stream, m_field_count, row,
                                      m_storage, is_eof);
           if (is_eof) m_current_state= EOF_PACKET;
           else {
...
void digest_field_packet(std::istream &is, Field_packet &field_packet) {
  ...
  // Protocol_chunk<boost::uint64_t>proto_default_value(
  // field_packet.default_value);
...
void digest_row_content(std::istream &is, int field_count,
                        Row_of_fields &row, String_storage &storage,
                        bool &is_eof) {
  boost::uint8_t size;
