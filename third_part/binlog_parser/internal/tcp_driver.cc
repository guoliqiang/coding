...
#include "third_part/boost/include/boost/thread.hpp"
#include <exception>
...
static int encrypt_password(
    boost::uint8_t *reply,  // buffer at least EVP_MAX_MD_SIZE
...
int Binlog_tcp_driver::connect(const std::string& user,
  ...
  if (!m_socket) {
    if ((m_socket=sync_connect_and_authenticate(m_io_service, user, passwd,
        host, port)) == 0) {
      return 1;
    }
  }
...
tcp::socket *sync_connect_and_authenticate(boost::asio::io_service &io_service,
  ...
  if (proto_read_package_header(socket, server_messages,
  ...
  boost::asio::read(*socket, server_messages,
  ...
  const char* env_libreplication_server_id =
      std::getenv("LIBREPLICATION_SERVER_ID");

  if (env_libreplication_server_id != 0) {
    try {
      boost::uint32_t libreplication_server_id =
          boost::lexical_cast<boost::uint32_t>(env_libreplication_server_id);
...
void Binlog_tcp_driver::start_binlog_dump(const std::string &binlog_file_name,
    size_t offset) {
  boost::asio::streambuf server_messages;
  std::ostream command_request_stream(&server_messages);
  ...
  // binlog position to start at
  Protocol_chunk<boost::uint32_t>prot_binlog_offset(offset);
  // not used
  Protocol_chunk<boost::uint16_t>prot_binlog_flags(binlog_flags);
  // must not be 0; see handshake package
  Protocol_chunk<boost::uint32_t>prot_server_id(server_id);
  ...
  if (!m_shutdown) GET_NEXT_PACKET_HEADER;

  // Start the event loop in a new thread
  if (!m_event_loop) {
    m_event_loop= new boost::thread(
        boost::bind(&Binlog_tcp_driver::start_event_loop, this));
  }
}

// Helper function used to extract the event header from a memory block
static void proto_event_packet_header(boost::asio::streambuf &event_src,
    Log_event_header *h) {
...
void Binlog_tcp_driver::handle_net_packet(const boost::system::error_code& err,
    std::size_t bytes_transferred) {
  if (err) {
    Binary_log_event * ev= create_incident_event(175, err.message().c_str(),
        m_binlog_offset);
  ...
  if (m_waiting_event->event_length == 0 &&
      m_event_stream_buffer.size() >= 19) {
  ...
  if (m_waiting_event->event_length ==
      m_event_stream_buffer.size() + LOG_EVENT_HEADER_SIZE - 1) {
...
void Binlog_tcp_driver::handle_net_packet_header(
  ...
  if (m_waiting_event == 0) {
    ...
    m_event_packet=  boost::asio::buffer_cast<char *>(
        m_event_stream_buffer.prepare(packet_length));
  }
...
int authenticate(tcp::socket *socket, const std::string& user, const std::string& passwd,
  ...
  try {
    ...
    Protocol_chunk<boost::uint8_t>prot_charset_number(
        handshake_package.server_language);
    Protocol_chunk<boost::uint8_t>prot_filler_buffer(filler_buffer, 23);
    Protocol_chunk<boost::uint8_t>prot_scramble_buffer_size(
        (boost::uint8_t) passwd_length);
    Protocol_chunk<boost::uint8_t>prot_scamble_buffer(
        (boost::uint8_t *)reply, passwd_length);
...
int Binlog_tcp_driver::wait_for_next_event(
    mysql::Binary_log_event **event_ptr) {
...
int Binlog_tcp_driver::set_position(const std::string &str,
  ...
  tcp::socket *socket;
  if ((socket= sync_connect_and_authenticate(io_service, m_user,
      m_passwd, m_host, m_port)) == 0) {
    return ERR_FAIL;
  }
  ...
  std::map<std::string, unsigned long >::iterator binlog_itr =
      binlog_map.find(str);

  // If the file name isn't listed on the server we will fail here.
  if (binlog_itr == binlog_map.end()) return ERR_FAIL;
...
int Binlog_tcp_driver::get_position(std::string *filename_ptr,
  ...
  boost::asio::io_service io_service;
  tcp::socket *socket;
  if ((socket = sync_connect_and_authenticate(io_service, m_user, m_passwd,
        m_host, m_port)) == 0) {
    return ERR_FAIL;
  }
  ...
  socket->close();
  delete socket;
  if (filename_ptr) *filename_ptr= m_binlog_file_name;
  if (position_ptr) *position_ptr= m_binlog_offset;
  return ERR_OK;
}
...
bool fetch_master_status(tcp::socket *socket, std::string *filename,
  ...
  // Send the request.
  boost::asio::write(*socket, boost::asio::buffer(command_packet_header, 4),
      boost::asio::transfer_at_least(4));
  boost::asio::write(*socket, server_messages,
      boost::asio::transfer_at_least(size));
...
bool fetch_binlogs_name_and_size(tcp::socket *socket, std::map<std::string,
  ...
  BOOST_FOREACH(Row_of_fields row, result_set) {
    ...
    binlog_map.insert(std::make_pair<std::string, unsigned long>(filename,
        (unsigned long)position));
  }
  return false;
}
...
int hash_sha1(boost::uint8_t *output, ...) {
  ...
  EVP_DigestFinal_ex(hash_context, (unsigned char *)output,
      (unsigned int *)&result);
  va_end(ap);
  return result;
}
...
int encrypt_password(boost::uint8_t *reply, // buffer at least EVP_MAX_MD_SIZE
  ...
  int length_reply = hash_sha1(reply,
                               ...
                               NULL, -1);
  int i;
  for ( i=0 ; i<length_reply ; ++i ) reply[i] = hash_stage1[i] ^ reply[i];
  return length_reply;
}
