...
static Binary_log_driver * parse_mysql_url(const char *body, size_t len) {
  ...
  if (*user_end == ':') {
    ...
    // There should be a password, but '@' was not found
    if (pass_end == 0) return 0;
  }
  ...
  if (host == host_end) return 0;  // No hostname was found
  // If no ':' was found there is no port, so the host end at the end of the
  // string
  if (host_end == 0) host_end = body + len;
  assert(host_end - host >= 1);  // There has to be a host

  // Find the port number
  unsigned long portno = 3306;
  if (*host_end == ':') portno = strtoul(host_end + 1, NULL, 10);

  // Host name is now the string [host, port-1) if port != NULL and [host, EOS)
  // otherwise. Port number is stored in portno, either the default, or a
  // parsed one
