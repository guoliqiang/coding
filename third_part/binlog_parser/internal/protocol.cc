...
void prot_parse_error_message(std::istream &is, struct st_error_package &err,
  ...
  // the remaining part of the package
  int message_size= packet_length - 2 - 1 - 5;
...
void proto_get_handshake_package(std::istream &is,
  ...
  Protocol_chunk<boost::uint16_t>proto_server_capabilities(
  ...
  if (remaining_bytes > 0) {
    Protocol_chunk<boost::uint8_t> proto_extension(extention_buffer,
        remaining_bytes);
    is >> proto_extension;
  }
}
