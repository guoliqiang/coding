...
int decimal_bin_size(int precision, int scale) {
  ...
  return(intg0 * sizeof(boost::int32_t) + dig2bytes[intg0x] +
...
float Value::as_float() const { return *((const float *)storage()); }
double Value::as_double() const { return *((const double *)storage()); }
