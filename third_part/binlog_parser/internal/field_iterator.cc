...
boost::uint32_t extract_metadata(const Table_map_event *map, int col_no) {
  ...
  for (int i=0; i < col_no; ++i) {
    ...
    offset += lookup_metadata_field_size(
        (enum mysql::system::enum_field_types)type);
  }

  boost::uint32_t metadata= 0;
  unsigned int type= (unsigned int)map->columns[col_no]&0xFF;
  switch(lookup_metadata_field_size(
      (enum mysql::system::enum_field_types)type)) {
    case 1:
...
int lookup_metadata_field_size(
    enum mysql::system::enum_field_types field_type) {
  switch(field_type) {
