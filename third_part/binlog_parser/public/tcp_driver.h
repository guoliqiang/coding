...
#include "third_part/binlog_parser/public/bounded_buffer.h"
...
class Binlog_tcp_driver : public Binary_log_driver {
 ...
 protected:
  // Connects to a mysql server, authenticates and initiates the event request
  // loop.
 ...
 private:
  ...
  void handle_net_packet_header(const boost::system::error_code& err,
                                std::size_t bytes_transferred);

  // Handles a completed network package with the assumption that it contains
  // a binlog event.
  // TODO rename to handle_event_log_packet?
  void handle_net_packet(const boost::system::error_code& err,
                         std::size_t bytes_transferred);
  ...
  void handle_event_packet(const boost::system::error_code& err,
                           std::size_t bytes_transferred);
  ...
  // We use this sturcture every time we initiate an async read.
