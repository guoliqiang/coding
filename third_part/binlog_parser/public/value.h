...
#ifndef THIRD_PART_BINLOG_PARSER_PUBLIC_VALUE_ADAPTER_H
#define	THIRD_PART_BINLOG_PARSER_PUBLIC_VALUE_ADAPTER_H
...
class Value {
 public:
  Value(enum enum_field_types type,
        boost::uint32_t metadata,
        const char *storage) : m_type(type), m_storage(storage),
                               m_metadata(metadata), m_is_null(false) {
    m_size = calc_field_size((unsigned char)type,
                             (const unsigned char*)storage, metadata);
  };
  ...
  // Returns the integer representation of a storage of a pre-specified type.
  boost::int32_t as_int32() const;
  // Returns the integer representation of a storage of pre-specified type.
...
#endif	// THIRD_PART_BINLOG_PARSER_PUBLIC_VALUE_ADAPTER_H
