...
#include "third_part/skip_list/public/atomic_pointer.h"
...
class SkipList {
 ...
 private:
  ...
  std::vector<base::shared_ptr<MemArea> > data_;
  Node* const head_;
...
SkipList<Key,Comparator>::NewNode(const Key& key, int height) {
  base::shared_ptr<MemArea> mem(new MemArea(
      sizeof(Node) + sizeof(AtomicPointer) * (height - 1)));
  data_.push_back(mem);
  return new (mem->Data()) Node(key);
}
...
SkipList<Key,Comparator>::FindLessThan(const Key& key) const {
  ...
  int level = GetMaxHeight() - 1;
  while (true) {
    CHECK(x == head_ || compare_(x->key, key) < 0);
    Node* next = x->Next(level);
