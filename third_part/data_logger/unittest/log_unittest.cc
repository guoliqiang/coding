...
std::string RandomSkewedString(int i, IRandom* rnd) {
...
class LogTest : public testing::Test {
 private:
  class StringDest : public WritableFile {
   public:
    ...
    virtual Status Append(const std::string& slice) {
  ...
  class StringSource : public SequentialFile {
   public:
    ...
    virtual Status Read(size_t n, std::string* result, char* scratch) {
      ...
      *result = std::string(contents_.data(), n);
      contents_.erase(0, n);
      return Status::OK();
    }
 ...
 public:
  ...
  void Write(const std::string& msg) {
    ...
    writer_.AddRecord(std::string(msg));
  }
  ...
  std::string Read() {
    if (!reading_) {
      reading_ = true;
      source_.contents_ = std::string(dest_.contents_);
    }
    std::string scratch;
    std::string record;
  ...
  void FixChecksum(int header_offset, int len) {
    ...
    uint32_t crc = crypto::Value(&dest_.contents_[header_offset+6], 1 + len);
    crc = crypto::Mask(crc);
    crypto::EncodeFixed32(&dest_.contents_[header_offset], crc);
  }
...
size_t LogTest::initial_offset_record_sizes_[] =
    {10000,  // Two sizable records in first block
     10000,
     2 * data_logger::kBlockSize - 1000,  // Span three blocks
     1};
...
uint64_t LogTest::initial_offset_last_record_offsets_[] =
    {0,
     ...
     2 * (kHeaderSize + 10000),
     2 * (kHeaderSize + 10000) +
         (2 * data_logger::kBlockSize - 1000) + 3 * kHeaderSize};
...
TEST_F(LogTest, RandomRead) {
  const int N = 500;
  IRandom write_rnd(301);
  for (int i = 0; i < N; i++) {
    Write(RandomSkewedString(i, &write_rnd));
  }
  IRandom read_rnd(301);
  for (int i = 0; i < N; i++) {
...
TEST_F(LogTest, ReadFourthFirstBlockTrailer) {
  CheckInitialOffsetRecord(data_logger::kBlockSize - 4, 3);
}

TEST_F(LogTest, ReadFourthMiddleBlock) {
  CheckInitialOffsetRecord(data_logger::kBlockSize + 1, 3);
}

TEST_F(LogTest, ReadFourthLastBlock) {
  CheckInitialOffsetRecord(2 * data_logger::kBlockSize + 1, 3);
}
