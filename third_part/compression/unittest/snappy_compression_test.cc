...
#include "util/compression/snappy_compression.h"
#include "testing/gtest/include/gtest/gtest.h"
#include "base/scoped_ptr.h"

namespace util {

TEST(SnappyCompression, CompressAndUncompress) {
  string test_data;
  const int kTestNum = 1000;
  for (int j = 0; j < kTestNum; j++) {
    test_data += rand() % 26 + 'a';
  }
  SnappyCompression compressor;
...
TEST(SnappyCompression, SSTableCompressAndUncompress) {
  SnappyCompression compressor;
