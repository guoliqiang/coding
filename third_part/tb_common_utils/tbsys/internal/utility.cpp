...
#include <netdb.h>
#include "third_part/tb_common_utils/tbsys/public/public_define.h"
#include "third_part/tb_common_utils/tbsys/public/utility.h"
...
// @return 0(成功)
// @return -1(失败)
int strJoin(char *pszDst, size_t sizeDst, char **ppszField,
            size_t sizeField, const char *pszSep) {
  int iFieldNum = 0;
  int iSepLen = 0;
  int i = 0;
  ...
  iSepLen = 0;
  if (pszSep != NULL) iSepLen = strlen(pszSep);
  for (i = 0; (size_t)i < sizeField; i++) {
    if ((i > 0) && (iSepLen > 0)) {
      if (sizeDst <= (size_t)iSepLen) return -1;
      strncat(pszDst, pszSep, sizeDst);
      sizeDst -= iSepLen;
    }
    iFieldNum = strlen(ppszField[i]);
    if (sizeDst <= (size_t)iFieldNum) return -1;
