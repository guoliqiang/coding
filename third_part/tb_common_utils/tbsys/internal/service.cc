...
#include "third_part/tb_common_utils/tbsys/public/ctrl_c_handler.h"
...
int Service::main(int argc,char*argv[]) {
  ...
  while (idx < argc) {
    if(strcmp(argv[idx],"-h") == 0 || strcmp(argv[idx],"--help") == 0) {
      ...
      if (_configFile.size() < 0 || _configFile.empty()) {
        cerr<<":--config|-f must be followed an argument,argument is not null"
            <<endl;
        return EXIT_FAILURE;
      }
      ...
      if (_cmd.size() < 0 || _cmd.empty()) {
        cerr<<":--cmd|-k must be followed an argument,argument is not null"
            <<endl;
        return EXIT_FAILURE;
      }
  ...
  if (_cmd.size() < 0 || _cmd.empty()) {
    cerr<<":must be with --cmd|-k,-cmd|-k must be followed "
          "an argument[start|stop] ,argument is not null"<<endl;
    return EXIT_FAILURE;
  }
