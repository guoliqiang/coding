...
// 构造函数
CFileQueueThread::CFileQueueThread(CFileQueue *queue, int threadCount,
    IQueueHandler *handler, void *args) : CDefaultRunnable(threadCount) {
  assert(queue != NULL);
  _queue = queue;
  _handler = handler;
  _args = args;
...
void CFileQueueThread::run(CThread *thread, void *args) {
  ...
  while(!_stop) {
    ...
    if (end_time > start_time) {
      TBSYS_LOG(INFO, "task execute speed: %d task/s",
                (int)((int64_t)1000000 * cnt / (end_time - start_time)));
      start_time = end_time;
    }
    _mutex.lock();
