...
queue_item *CFileQueue::pop(uint32_t index) {
  ...
  while(retryReadCount<3) {
    ...
    if (retSize < 0) {
      TBSYS_LOG(ERROR, "读出错, m_readFd:%d, %s(%d)",
                m_readFd, strerror(errno), errno);
      break;
    }
    if (retSize == sizeof(int)) {
      ...
      if (size < (int)sizeof(queue_item) || size > (int)TBFQ_MAX_FILE_SIZE) {
        ...
        TBSYS_LOG(WARN, "读出的size不正确:%d,curPos:%d,readOff:%d,"
                        "retry:%d,seqno:%u,m_readFd:%d", 
                  size, curPos, m_head.read_offset, retryReadCount,
                  m_head.read_seqno,m_readFd);
        retryReadCount ++;
      ...
      if ((ret = read(m_readFd, item, size)) != size) {
        ...
        TBSYS_LOG(WARN, "读文件不正确:%d<>%d,curPos:%d,readOff:%d,"
                        "retry:%d,seqno:%u,m_readFd:%d", 
                  ret, size, curPos, m_head.read_offset, retryReadCount,
                  m_head.read_seqno,m_readFd);
        retryReadCount ++;
        free(item);
        item = NULL;
      ...
      if (item->flag != TBFQ_FILE_QUEUE_FLAG) {
        // 重新读
        int64_t curPos = lseek(m_readFd, 0-ret-retSize, SEEK_CUR);
        TBSYS_LOG(WARN, "flag不正确:item->flag(%d)<>FLAG(%d),curPos:%d,"
                        "readOff:%d,retry:%d,seqno:%u,m_readFd:%d", 
      ...
      if (item->len + (int)sizeof(queue_item) != size) {
        // 重新读
        int64_t curPos = lseek(m_readFd, 0-ret-retSize, SEEK_CUR);
        TBSYS_LOG(WARN, "读出的len不正确:%d<>%d,curPos:%d,readOff:%d,retry:%d"
                         ",seqno:%u,m_readFd:%d", 
                  item->len + sizeof(queue_item), size, curPos,
                  m_head.read_offset, retryReadCount,
                  m_head.read_seqno,m_readFd);
        retryReadCount ++;
        free(item);
        item = NULL;
...
void CFileQueue::backup(uint32_t index) {
  ...
  sprintf(cmd, "cp -f %s/%08u.dat %s/%08u.dat.backup >/dev/null 2>&1 0>&1", 
          ...
          m_queuePath, m_head.read_seqno);
   int rest = system(cmd);
   TBSYS_LOG(WARN, "%s, readpos: %u:%d, currpos: %u:%d, curPos: %d, "
                   "m_readFd: %d result: %d", cmd, 
