...
class TestTask : public TimerTask, Monitor<Mutex> {
 public:
  ...
  virtual tbutil::Time getRunTime() const {
    Lock sync(*this);
    return _run;
  }

  tbutil::Time getScheduledTime() const {
...
class DestroyTask : public TimerTask, Monitor<Mutex> {
 public:
  DestroyTask(const tbutil::TimerPtr& timer) : _timer(timer), _run(false) { }
  virtual void runTimerTask() {
    Lock sync(*this);
    _timer->destroy();
    _run = true;
...
int main(int argc, char* argv[]) {
  ...
  cout<<" time string : "<<t.toDateTime()<<endl<<" time : "
    <<t.toDuration()<<endl;
