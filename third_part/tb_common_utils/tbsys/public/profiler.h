...
do { \
  if(tbsys::util::Profiler::m_profiler.status == 1) \
    tbsys::util::Profiler::m_profiler.start((s)); \
} while(0)

#define PROFILER_STOP() \
do { \
  if(tbsys::util::Profiler::m_profiler.status == 1) \
    tbsys::util::Profiler::m_profiler.reset(); \
} while (0)
#define PROFILER_BEGIN(s) \
do { \
  if(tbsys::util::Profiler::m_profiler.status == 1) \
    tbsys::util::Profiler::m_profiler.begin((s)); \
} while(0)
#define PROFILER_END() \
do { \
  if(tbsys::util::Profiler::m_profiler.status == 1) \
    tbsys::util::Profiler::m_profiler.end(); \
} while(0)
#define PROFILER_DUMP() \
do { \
  if(tbsys::util::Profiler::m_profiler.status == 1) \
    tbsys::util::Profiler::m_profiler.dump(); \
} while(0)
#define PROFILER_SET_THRESHOLD(sh) \
tbsys::util::Profiler::m_profiler.threshold = (sh)
#define PROFILER_SET_STATUS(st) \
tbsys::util::Profiler::m_profiler.status = (st)
...
//
// 配置参数：
// 设置计数器的状态，如果不是1，则禁用计数器所有功能，
// 此时不会产生任何开销，默认为1
// PROFILER_SET_STATUS(status);
// 设置dump的阀值，当一个计数实例的总计时超过这个阀值时才会dump信息，
// 单位为us，默认为10000us(10ms)
...
class Entry {
 public:
  ...
  ~Entry() {
    if (!subEntries.empty()) {
      for (size_t i=0; i<subEntries.size(); i++) delete subEntries[i];
    }
  }
  ...
  long getDuration() {
    if (etime >= stime) return (etime - stime);
    else return -1;
  }

  long getEndTime() {
    if (etime >= btime) return (etime - btime);
    else return -1;
  }
  ...
  long getMyDuration() {
    long td = getDuration();
    if (td < 0) return -1;
    else if (subEntries.empty()) return td;
    else {
  ...
  double getPercentage() {
    ...
    if (!subEntries.empty()) pd = getDuration();
    else if (parent && parent->isReleased())
    pd = parent->getDuration();
    if (pd > 0 && d > 0) return d / pd;
    return 0;
  }
  ...
  double getPercentageOfTotal() {
    ...
    if (first && first->isReleased()) fd = first->getDuration();
    if (fd > 0 && d > 0) return d / fd;
    return 0;
  }
  ...
  Entry *getUnreleasedEntry() {
    if(subEntries.empty()) return NULL;
    Entry *se = subEntries.back();
    if(se->isReleased()) return NULL;
    else return se;
  }
  ...
  std::string toString(const std::string& pre1, const std::string& pre2,
    ...
    if (isReleased()) {
      ...
      sprintf(temp, "%lu [%lu(us), %lu(us), %.2f%%, %.2f%%] - %s",
              ...
              getPercentage() * 100, getPercentageOfTotal() * 100,
              message.c_str());
      ss << temp;
    } else {
      ss << "[UNRELEASED]";
    ...
    for (size_t i=0; i<subEntries.size(); i++) {
      ...
      ss << std::endl;
      if (i == 0) ss << ent->toString(pre2 + "+---", pre2 + "|  ");
