...
#include <set>
#include <map>
#include "third_part/tb_common_utils/tbsys/public/shared.h"
...
class Timer :public virtual Shared ,private virtual tbutil::Thread {
 ...
 private:
  ...
  class TimerTaskCompare :
      public std::binary_function<TimerTaskPtr, TimerTaskPtr, bool> {
   public:
...
inline Timer::Token::Token(const Time& st, const Time& d,
    const TimerTaskPtr& t) : scheduledTime(st), delay(d), task(t) { }
