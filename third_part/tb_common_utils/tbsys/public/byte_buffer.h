...
class ByteBuffer {
 ...
 public:
  ...
  ByteBuffer & put(const std::string & e) throw (out_of_range) {
    return putString(e);
  }
  ByteBuffer & get(std::string & e) throw (out_of_range) {
    return getString(e);
  }
  ByteBuffer & putString(const std::string & v) throw (out_of_range);
  ByteBuffer & getString(std::string & v) throw (out_of_range);
  // get data_ directly..
  virtual ByteBuffer & put(const char * src,
                           uint32_t offset,
                           uint32_t size)  throw (out_of_range);
  virtual ByteBuffer & get(char* dst,
                           uint32_t offset,
                           uint32_t size) throw (out_of_range);
  // relative get method, from index of data_
  ByteBuffer & get(int index, char* dst,
                   uint32_t offset, uint32_t size) throw (out_of_range);
  // fetch data_ directly, use them very carefully 
  ByteBuffer & getRef(int index, const char* &dst,
                      uint32_t size) throw (out_of_range);
  const ByteBuffer & rawData(int index, const char* & dst,
...
template <typename T>
ByteBuffer & ByteBuffer::put(const T & e) throw (ByteBuffer::out_of_range) {
...
template <typename T>
ByteBuffer & ByteBuffer::get(T & e) throw (ByteBuffer::out_of_range) {
  if (position_ + sizeof(T) > size_) 
    throw out_of_range(position_, sizeof(T), size_);
  peek(e);
  position_ += sizeof(T);
...
template <typename T>
T ByteBuffer::get() throw (ByteBuffer::out_of_range) {
  T e;
  get(e);
  return e;
}
...
template <typename T>
ByteBuffer & ByteBuffer::getRef(int index, T* &dst) throw (out_of_range) {
  const char* ref = 0;
...
template <typename T> 
ByteBuffer & ByteBuffer::getRef(int index, const T* &dst) throw (out_of_range) {
  const char* ref = 0;
  getRef(index, ref, sizeof(T));
  dst = (const T*) ref;
  return *this;
