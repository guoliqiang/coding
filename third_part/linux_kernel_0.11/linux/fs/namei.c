...
static int permission(struct m_inode * inode,int mask) {
  ...
  // special case: not even root can read/write a deleted file
	if (inode->i_dev && !inode->i_nlinks) return 0;
	else if (current->euid==inode->i_uid) mode >>= 6;
	else if (current->egid==inode->i_gid) mode >>= 3;
...
static struct m_inode * get_dir(const char * pathname) {
	...
	else return NULL;	// empty name is bad
	inode->i_count++;
	while (1) {
		thisname = pathname;
