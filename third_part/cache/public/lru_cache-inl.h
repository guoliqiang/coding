...
#ifndef UTIL_CACHE_LRU_CACHE_INL_H_
#define UTIL_CACHE_LRU_CACHE_INL_H_
...
base::shared_ptr<TypeValue> LRUCache<TypeKey, TypeValue>::Get(
  ...
  if (iter != index_.end()) {
    ...
    // http://blog.sina.com.cn/s/blog_60b5d6bc0100iksq.html
...
void LRUCache<TypeKey, TypeValue>::Put(const TypeKey &key,
                                       base::shared_ptr<TypeValue> value) {
  {
    ...
    if (iter != index_.end()) {
      ...
      if (value.get() != NULL &&
          iter->second->second.get() == value.get())
        return;
      RemoveValue(key);
    }
  }
...
#endif  // UTIL_CACHE_LRU_CACHE_INL_H_
