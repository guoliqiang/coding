...
#ifndef TRIE_TREE_UNIT_H_
#define TRIE_TREE_UNIT_H_
...
class TrieTreeUnit {
 ...
 private:
  struct NodeUnit {
    NodeUnit() {
      parent = NULL;
      key_table = new map<Key, NodeUnit*>();
      values = new set<Val>();
    };
    ~NodeUnit() {
      delete values;
      typename map<Key, NodeUnit*>::iterator it = key_table->begin();
    ...
    Key ch;
    NodeUnit* parent;
    map<Key, NodeUnit*>* key_table;
    set<Val>* values;
  };
  void RemoveNodeUnit(NodeUnit* node);
  int CountMinTrie(NodeUnit* node) const;
  ...
  int min_trie_;
  set<Val> empty_set_;
  DISALLOW_COPY_AND_ASSIGN(TrieTreeUnit);
};

}  // namespace trie_tree

#endif  // TRIE_TREE_UNIT_H_
