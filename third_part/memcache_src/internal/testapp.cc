...
#include "third_part/memcache_src/public/config.h"
#include "third_part/memcache_src/public/cache.h"
#include "third_part/memcache_src/public/util.h"
#include "third_part/memcache_src/public/protocol_binary.h"
...
static enum test_return test_binary_delete_impl(const char *key, uint8_t cmd) {
  ...
  validate_response_header(&receive.response, cmd,
      PROTOCOL_BINARY_RESPONSE_KEY_ENOENT);
  len = storage_command(send.bytes, sizeof(send.bytes),
      PROTOCOL_BINARY_CMD_ADD, key, strlen(key), NULL, 0, 0, 0);
  safe_send(send.bytes, len, false);
  safe_recv_packet(receive.bytes, sizeof(receive.bytes));
  validate_response_header(&receive.response, PROTOCOL_BINARY_CMD_ADD,
      PROTOCOL_BINARY_RESPONSE_SUCCESS);
    ...
    switch (cmd) {
      ...
      case PROTOCOL_BINARY_CMD_SETQ:
        len = storage_command(command.bytes, sizeof(command.bytes), cmd,
            key, keylen , &value, sizeof(value), 0, 0);
        break;
      case PROTOCOL_BINARY_CMD_APPEND:
      case PROTOCOL_BINARY_CMD_APPENDQ:
      case PROTOCOL_BINARY_CMD_PREPEND:
