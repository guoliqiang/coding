...
// Lock for global stats gc
static pthread_mutex_t stats_lock;
// Free list of CQ_ITEM structs gc
static CQ_ITEM *cqi_freelist;
static pthread_mutex_t cqi_freelist_lock;

static pthread_mutex_t *item_locks;
// size of the item lock hash table gc
static uint32_t item_lock_count;
#define hashsize(n) ((unsigned long int)1<<(n))
#define hashmask(n) (hashsize(n)-1)
// this lock is temporarily engaged during a hash table expansion gc
static pthread_mutex_t item_global_lock;
// thread-specific variable for deeply finding the item lock type gc
...
// Convenience functions for calling *only* when in ITEM_LOCK_GLOBAL mode gc
...
void *item_trylock(uint32_t hv) {
  pthread_mutex_t *lock =
      &item_locks[(hv & hashmask(hashpower)) % item_lock_count];
  if (pthread_mutex_trylock(lock) == 0) return lock;
  return NULL;
}
...
void switch_item_lock_type(enum item_lock_types type) {
  ...
  for (i = 0; i < settings.num_threads; i++) {
    if (write(threads[i].notify_send_fd, buf, 1) != 1) {
      ...
      // TODO: This is a fatal problem. Can it ever happen temporarily? gc
    }
  }
...
static CQ_ITEM *cqi_new(void) {
  ...
  if (NULL == item) {
    ...
    // the freelist.
    for (i = 2; i < ITEMS_PER_ALLOC; i++) item[i - 1].next = &item[i];
    pthread_mutex_lock(&cqi_freelist_lock);
