...
int get_server(struct ketama *ring, const char *key) {
	...
	while (h) {
		midp = (unsigned int)(lowp + (highp - lowp) / 2);
		if (midp == maxp) return ring->dot[0].srvid;

		midval = ring->dot[midp].point;
		midval1 = (midp == 0? 0 : ring->dot[midp - 1].point);

		if (h <= midval && h > midval1) return ring->dot[midp].srvid;

		if (midval < h) lowp = midp + 1;
		else highp = midp - 1;

		if (lowp > highp) return ring->dot[0].srvid;
	}
	return -1;
}
...
void free_ketama(struct ketama *k) {
	...
	if (k->dot) free(k->dot);
	if (k->name) {
		for (i = 0; i < k->count; i++) free(k->name[i]);
		free(k->name);
	}
	free(k->weight);
	free(k);
