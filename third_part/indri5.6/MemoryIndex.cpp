...
int indri::index::MemoryIndex::documentLength(lemur::api::DOCID_T documentID) {
  ...
  if (base > documentID || (documentID - base) > (int)_documentData.size()) return 0;
...
std::string indri::index::MemoryIndex::term(lemur::api::TERMID_T termID) {
  if (termID <= 0 || termID > (int)_idToTerm.size()) return std::string();
...
std::string indri::index::MemoryIndex::field(int fieldID) {
  if (fieldID <= 0 || fieldID > (int)_fieldData.size()) return "";
...
void indri::index::MemoryIndex::_writeDocumentTermList(UINT64& offset, int& byteLength,
                                                       lemur::api::DOCID_T documentID, int documentLength,
...
void indri::index::MemoryIndex::_writeDocumentStatistics(UINT64 offset, int byteLength,
                                                         int indexedLength, int totalLength, int uniqueTerms) {
...
void indri::index::MemoryIndex::_addOpenTags(indri::utility::greedy_vector<indri::parse::TagExtent *>& indexedTags,
  ...
  for (; extentIndex < extents.size(); extentIndex++) {
    ...
    if (extent->begin > (int)position) break;
    int tagId = _fieldID(extent->name);
    if (tagId == 0) continue;
...
lemur::api::DOCID_T indri::index::MemoryIndex::addDocument(indri::api::ParsedDocument& document) {
  ...
  for (position = 0; position < words.size(); position++) {
    ...
    for (indri::utility::greedy_vector<indri::parse::TagExtent *>::iterator tag = openTags.begin();
         tag != openTags.end(); tag++) {
  ...
  std::cout << "add file:" << documentID << std::endl; 
  return documentID;
}
...
indri::index::DocExtentListIterator* indri::index::MemoryIndex::fieldListIterator(int fieldID) {
  if (fieldID <= 0 || fieldID > (int)_fieldData.size()) return 0;
...
const indri::index::TermList* indri::index::MemoryIndex::termList(lemur::api::DOCID_T documentID) {
  ...
  if (documentIndex < 0 || documentIndex >= (int)_documentData.size()) return 0;
...
size_t indri::index::MemoryIndex::memorySize() {
  ...
  return listDataSize + documentDataSize + termListsSize + fieldListsSize;
}
