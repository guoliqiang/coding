...
// if contain, return index of key, otherwise return index of first smaller than key,
// if key is the smallest return -1
...
bool indri::file::BulkBlock::getIndex(int index, char* key, int& keyActual,
                                      int keyLength, char* value,
                                      int& valueActual, int valueLength) {
...
bool indri::file::BulkBlock::findGreater(const char* key, int keyLength, char* value,
                                         int& actualLength, int valueBufferLength) {
  bool exact;
...
bool indri::file::BulkBlock::find(const char* key, int keyLength,
                                  char* value, int& actualLength, int valueBufferLength) {
  bool exact;
  int index = _find(key, keyLength, exact);
  actualLength = 0;
  if (index < 0 || !exact) return false;
  int keyActual;
  return getIndex(index, 0, keyActual, 0, value, actualLength, valueBufferLength);
}
...
void indri::file::BulkBlock::link(indri::file::BulkBlock* previous, indri::file::BulkBlock* next) {
  _previous = previous;
  _next = next;
  if (previous) previous->_next = this;
...
indri::file::BulkTreeWriter::BulkTreeWriter() : _write(_file, 1024*1024) {
  _blockID = 0;
  _blocks.push_back(new BulkBlock(true));  // only the first is leaf
  _flushLevel = 0;
}
...
void indri::file::BulkTreeWriter::_flushAll() {
  ...
  for (int i = 0; i < (int)_blocks.size(); i++) {
    ...
    if (count == 1 && hasNotBeenFlushed) break;
    if (count) _flush(i);
  }
}
...
indri::file::BulkBlock* indri::file::BulkTreeReader::_fetch(UINT32 id) {
  ...
  if (!result) {
    ...
    block = *result;
    if (_tail == block) _tail = block->previous();
  }

  // move to front of list
  block->unlink();
  block->link(0, _head);
  if (_tail == 0) _tail = block;
  _head = block;
...
bool indri::file::BulkTreeIterator::get(char* key, int keyLength, int& keyActual,
