...
static void set_error(struct fcb *f, int err, const char caption[])
{
...
static void set_error1(struct fcb *f, int err, const char caption[], int code)
{
  set_err(f,(UINT32)err);
  fn_log_f("%s%d\n",caption,code);
...
static void set_error2(struct fcb *f, int err, const char caption[], int code1, int code2)
{
  set_err(f,(UINT32)err);
  fn_log_f("%s%d/%d\n",caption,code1,code2);
...
{char name[max_filename_lc+10]; char mode[30];
...
static struct leveln_pntr search_index(struct fcb *f, int index, UINT32 stop_lvl,
  struct key *k)
{struct leveln_pntr child; int ix,bufix; boolean done=false,found; char name[30]="search_index";
...
{int bufix,ix; boolean found,at_end,update_parent; struct key old_max_key,new_separator;
 struct leveln_pntr b; char name[30]="ix_delete";
...
{int ix,bufix; unsigned new_prefix_lc,new_lc; char name[50]="rep_maxkey&pntr";
...
static boolean check_ix_block_after_move(struct fcb *f, struct ix_block *b, const char caption[],
...
static void print_shuffle_candidate(FILE *list, const char caption[], struct shuffle_candidate *c)
{
...
{int bufix,ix; boolean found; char name[30]="update_index";
...
boolean found=false,seq=false;
 unsigned char t1[maxkey_lc]; char name[30]="kf_get_ptr";
...
{int ix,bufix; unsigned lc,rec_lc; boolean have_space=false,found;
 unsigned char t1[maxkey_lc]; char name[30]="kf_put_rec";
