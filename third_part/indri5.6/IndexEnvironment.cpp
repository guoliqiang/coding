...
std::vector<indri::parse::Transformation*>
indri::api::IndexEnvironment::_createAnnotators(const std::string& fileName,
...
indri::api::IndexEnvironment::_applyAnnotators(std::vector<indri::parse::Transformation*>& annotators,
...
indri::api::IndexEnvironment::IndexEnvironment() : _callback(0),
                                                   _options(0),
                                                   _documentsIndexed(0),
                                                   _documentsSeen(0) {}
...
void indri::api::IndexEnvironment::addFileClass(const std::string& name, 
  ...
  this->_fileClassFactory.addFileClass(name, iter, parser, tokenizer,
                                       startDocTag, endDocTag, endMetadataTag,
...
void indri::api::IndexEnvironment::setMetadataIndexedFields(const std::vector<std::string>& forwardFields,
                                                            const std::vector<std::string>& backwardFields) {
  if (!_parameters.exists("collection")) _parameters.set("collection", "");
...
void indri::api::IndexEnvironment::addFile(const std::string& fileName, const std::string& fileClass) {
  ...
  if (!parser || !iterator) {
    _documentsSeen++;
    if (_callback) {
      (*_callback) (indri::api::IndexStatus::FileSkip, fileName, _error, _documentsIndexed, _documentsSeen);
    }
  } else {
    try {
      ...
      while (document = iterator->nextDocument()) {
        ...
        if (_blackedDocs.find(docIDStr) != _blackedDocs.end()) continue;
        ...
        if (_callback) {
          (*_callback)(indri::api::IndexStatus::DocumentCount, fileName, _error, _documentsIndexed, _documentsSeen);
        }
      }
      // notify caller that the file was successfully closed
      if (_callback) { 
        (*_callback)(indri::api::IndexStatus::FileClose, fileName, _error, _documentsIndexed, _documentsSeen);
      }
      iterator->close();
...
lemur::api::DOCID_T
indri::api::IndexEnvironment::addString(const std::string& documentString, const std::string& fileClass,
  ...
  _documentsIndexed++;
  if (_callback) {
    (*_callback)(indri::api::IndexStatus::DocumentCount, nothing, _error, _documentsIndexed, _documentsSeen);
  }
  return documentID;
}
...
//
lemur::api::DOCID_T
indri::api::IndexEnvironment::addString(const std::string& documentString,
                                        const std::string& fileClass,
                                        const std::vector<indri::parse::MetadataPair>& metadata,
...
lemur::api::DOCID_T
indri::api::IndexEnvironment::addParsedDocument(ParsedDocument* document) {
  std::string nothing;
  _documentsSeen++;
