...
class Parameters {
   ...
   public:
    ...
    parameter_value(const parameter_value& other) {
      ...
      for (MValue::const_iterator iter = other.table.begin();
           iter != other.table.end(); iter++) {
        table.insert(std::make_pair(iter->first, new parameter_value(*(iter->second))));
      }
    }
 ...
 public:
  ...
  operator int () {
    const std::string& value = _getRoot()->getValue();
    if (value.length() == 0) LEMUR_THROW(LEMUR_BAD_PARAMETER_ERROR, "Empty parameter text");
    if (_isBoolean(value)) return _asBoolean(value);
  ...
  const Parameters& operator = (const Parameters& other) {
    ...
    for (iter = other._collection->table.begin();
         iter != other._collection->table.end(); iter++) {
      _collection->table.insert(std::make_pair(iter->first, new parameter_value(*iter->second)));
    }
