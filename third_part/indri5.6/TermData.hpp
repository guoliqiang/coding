...
struct TermData {
 ...
 public:
  TermData() : maxDocumentLength(0), minDocumentLength(MAX_INT32) {
    term = 0;
  }
...
inline void termdata_compress(indri::utility::RVLCompressStream& stream, indri::index::TermData* termData, int fieldCount) {
  // corpus statistics
  stream << termData->corpus.totalCount << termData->corpus.documentCount;
  // max-score statistics
  stream << termData->maxDocumentLength << termData->minDocumentLength;
  // field statistics
  for (int i = 0; i < fieldCount; i++) {
    stream << termData->fields[i].totalCount << termData->fields[i].documentCount;
  }
}
