...
indri::api::Parameters& indri::api::Parameters::instance() {
  if (!_parametersSingleton) _parametersSingleton = new indri::api::Parameters();
  return *_parametersSingleton;
}

void indri::api::Parameters::_parseNextSegment(std::string& segment, int& arrayIndex,
                                               int& endOffset, const std::string& path, int beginOffset) {
  endOffset = (int)path.find('.', beginOffset);
  arrayIndex = -1;

  if (endOffset < 0) segment = path.substr(beginOffset);
  else segment = path.substr(beginOffset, endOffset);
...
indri::api::Parameters::parameter_value* indri::api::Parameters::_getSegment(const std::string& segment, int arrayIndex,
                                                                             indri::api::Parameters::parameter_value* from) {
...
indri::api::Parameters::parameter_value* indri::api::Parameters::_getPath(const std::string& path,
                                                                          indri::api::Parameters::parameter_value* last, int offset) {
  int endOffset; 
  int arrayIndex = 0;
  std::string segment;
...
indri::api::Parameters::Parameters() : _collection(new parameter_value), _owned(true) { }

indri::api::Parameters::Parameters(const Parameters& other) : _collection(other._collection), _owned(false) {}

indri::api::Parameters::Parameters(parameter_value* value) : _collection(value), _owned(false) { }

indri::api::Parameters::~Parameters() {
  if (_owned) delete _collection;
}

indri::api::Parameters indri::api::Parameters::get(size_t index) {
  if (!exists(index)) LEMUR_THROW(LEMUR_IO_ERROR, "Required index didn't exist.");

  assert(exists(index));
  parameter_value* root = _getRoot();

  if (root->array.size()) return Parameters(root->array[index]);
  else return Parameters(root);
}
...
indri::api::Parameters indri::api::Parameters::get(const std::string& name) {
  assert(exists(name));
  if (!exists(name)) LEMUR_THROW(LEMUR_IO_ERROR, "Required parameter '" + name + "' was not specified.");
...
size_t indri::api::Parameters::size() {
  parameter_value* root = _getRoot();
  if (!root) return 0;
  if (root->array.size()) return root->array.size();
  if (root->value.length()) return 1;
  if (root->table.size()) return 1;
  return 0;
}
...
void indri::api::Parameters::_loadXML(indri::xml::XMLNode* node) {
  ...
  for(size_t i=0; i<children.size(); i++) {
    ...
    if (seen.find(name) == seen.end()) seen.insert(name);
    else arrays.insert(name);
    if (child->getAttribute("append") == "true") appends.insert(name);
  }
  ...
  for(iter = seen.begin(); iter != seen.end(); iter++) {
    if (appends.find(*iter) != appends.end()) continue;
    if (exists(*iter) && 
        get(*iter).size() > 1 ||
