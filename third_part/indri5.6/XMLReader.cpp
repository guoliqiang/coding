...
int indri::xml::XMLReader::_readTag(const char* buffer, int bufferStart, int bufferEnd,
                                    std::string* tagName, std::map<std::string, std::string>* attributes,
                                    int* tagType) {
  ...
  if (buffer[position] == '/') {
    ...
    if (buffer[endLocation-1] == '/') {
      if (tagType) *tagType = TAG_OPEN_CLOSE_TYPE;
      endLocation--;
    } else if (tagType) {
  ...
  if (tagName || attributes) {
    ...
    if (tagName) tagName->assign(&buffer[textBegin], &buffer[textEnd]);
...
int indri::xml::XMLReader::_findClosingTag(const char* buffer, int start, int finish,
                                           std::string& openingTagName, bool* tagsBetween) {
  int openingTags = 0;
  int closingTags = 0;
  int position = start;
  bool done = false;
  ...
  if (tagsBetween) *tagsBetween = false;
  try {
    while (!done) {
      std::string tagName;
      ...
      if (tagType == TAG_CDATA_TYPE) {
        ...
        if (tagsBetween) *tagsBetween = true;

        if (tagType == TAG_OPEN_TYPE) openingTags++;
        position = end;
...
void indri::xml::XMLReader::_read(indri::xml::XMLNode** parent, const char* buffer, int start, int end) {
  ...
  for (int current = _tryFindBeginTag(buffer, start, end);
       current >= 0; current = _tryFindBeginTag(buffer, current, end)) {
