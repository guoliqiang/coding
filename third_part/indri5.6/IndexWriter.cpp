...
void IndexWriter::_writeSkip(indri::file::SequentialWriteBuffer* buffer,
                             lemur::api::DOCID_T document, int length) {
...
void IndexWriter::_writeBatch(indri::file::SequentialWriteBuffer* buffer,
                              lemur::api::DOCID_T document, int length, indri::utility::Buffer& data) {
...
void IndexWriter::_writeFieldLists(std::vector<WriterIndexContext*>& contexts, const std::string& path) {
  ...
  for (size_t field = 0; field < _fields.size(); field++) {
    ...
    _fieldData.push_back(FieldStatistics(_fields[field].name, _fields[field].numeric, _fields[field].ordinal,
                                         _fields[field].parental, 0, 0, fieldsOutput->tell()));
...
void IndexWriter::_pushInvertedLists(indri::utility::greedy_vector<WriterIndexContext*>& lists, invertedlist_pqueue& queue) {
  for (size_t i = 0; i < lists.size(); i++) {
    ...
    if (!lists[i]->iterator->finished()) queue.push(lists[i]);
  }
}

// _writeStatistics
void IndexWriter::_writeStatistics(indri::utility::greedy_vector<WriterIndexContext*>& lists,
                                   indri::index::TermData* termData, UINT64& startOffset) {
  ...
  for (size_t i=0; i<iterators.size(); i++) {
    ...
    while (!iterator->finished()) {
      ...
      for (int j=0; j<count; j++) {
        ...
        if (entry->numbers.size()) stream << entry->numbers[j];
      }
      iterator->nextEntry();
      documents++;
    }
...
void IndexWriter::_storeMatchInformation(indri::utility::greedy_vector<WriterIndexContext*>& lists, int sequence,
                                         indri::index::TermData* termData, UINT64 startOffset, UINT64 endOffset) {
  bool isFrequent = termData->corpus.totalCount > FREQUENT_TERM_COUNT;
  if (isFrequent) _isFrequentCount++;
  ...
  for (size_t i = 0; i < lists.size(); i++) {
    ...
    if (!wasFrequent) list->wasInfrequentCount++;
    if (wasFrequent) list->wasFrequentCount++;
    list->sequenceCount++;
...
indri::index::TermTranslator* IndexWriter::_buildTermTranslator(indri::file::BulkTreeReader& newInfrequentTerms,
  ...
  if (frequent->size() == 0) frequent->resize(1);
  (*frequent)[0] = 0;
